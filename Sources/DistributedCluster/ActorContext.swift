...
public class _ActorContext<Message: Codable> /* TODO(sendable): NOTSendable*/ {
    ...
    @usableFromInline
    internal func makeAsynchronousCallback<T>(file: String = #fileID, line: UInt = #line, _ callback: @escaping (T) throws -> Void) -> AsynchronousCallback<T> {
    ...
    /// - Returns: an `AsynchronousCallback` that is safe to call from outside of this actor
    @usableFromInline
    internal func makeAsynchronousCallback<T>(for type: T.Type, file: String = #fileID, line: UInt = #line, callback: @escaping (T) throws -> Void) -> AsynchronousCallback<T> {
        AsynchronousCallback(callback: callback) { [weak selfRef = self.myself._unsafeUnwrapCell] in
            selfRef?.sendClosure(file: file, line: line, $0)
        }
    }
    ...
    internal func onResultAsync<AR: _AsyncResult>(of _AsyncResult: AR, timeout: Duration, file: String = #fileID, line: UInt = #line, _ continuation: @escaping (Result<AR.Value, Error>) throws -> _Behavior<Message>) {
